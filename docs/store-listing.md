...
The screenshots that go on the same page live in `store-assets/`, and are generated
from the built extension by `npm run screenshots`. Their captions repeat claims made
below - the language count, the export formats - so the two are edited together.
